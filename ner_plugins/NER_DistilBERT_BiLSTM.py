...
class DistilBERT_BiLSTM(torch.nn.Module):
  def __init__(self):
        ...
        member parameters.
        """
        super(DistilBERT_BiLSTM, self).__init__()
        # hidden size of BERT, hidden size of our classifier, and number of labels
        D_in, H, D_out = 768, 256, 10 # 4000
      
        self.embedding = DistilBertModel.from_pretrained(
        ...
        # In this model we only train BiLSTM embedding and linear classifier, DistilBERT embedding stands frozen
...
class NER_DistilBERT_BiLSTM():
  ...
  def __init__(self):
    ...
    self.model = DistilBERT_BiLSTM()
    if os.path.exists("Models/NER_DistilBERT_BiLSTM.pt"):
      print("Loading model") 
      self.model.load_state_dict(torch.load("Models/NER_DistilBERT_BiLSTM.pt", map_location=torch.device('cpu')))
      print("Loaded model")

    self.tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-cased', num_labels=len(NER_DistilBERT_BiLSTM.tag2idx), do_lower_case=False)
  ...
  def transform_sequences(self,tokens_labels):
    ...
    input_ids = pad_sequences([self.tokenizer.convert_tokens_to_ids(txt) for txt in tokenized_sentences],
                            maxlen=NER_DistilBERT_BiLSTM.MAX_LEN, dtype="long", value=0.0,
                            truncating="post", padding="post")

    tags = pad_sequences([[NER_DistilBERT_BiLSTM.tag2idx.get(l) for l in lab] for lab in labels],
                        maxlen=NER_DistilBERT_BiLSTM.MAX_LEN, value=NER_DistilBERT_BiLSTM.tag2idx["PAD"], padding="post",
  ...
  def learn(self, X_train,Y_train, epochs=1):
    ...
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=NER_DistilBERT_BiLSTM.bs)
    ...
    for _ in trange(epochs, desc="Epoch"):
        ...
        for step, batch in enumerate(train_dataloader):
            # add batch to gpu
            batch = tuple(b.to(NER_DistilBERT_BiLSTM.device) for b in batch)
            ...
            print("OUTPUTS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            # TODO: Issue with the model itself, in that case with baseline distilbert


            # b_input_ids = torch.tensor([[101, 7592, 102]]).type(torch.long)
            # b_input_ids.to(NER_DistilBERT_BiLSTM.device)
            # b_input_mask = torch.tensor([[1,1,1]]).type(torch.long)
            # b_input_mask.to(NER_DistilBERT_BiLSTM.device)
            
            
            # outputs = self.model.distilbert(b_input_ids,
        ...
        torch.save(self.model.state_dict(), os.path.join("Models_intermediate/", 'DistilBERT_BiLSTM_epoch-{}.pt'.format(epoch_num)))
  ...
  def evaluate(self, X_test,Y_test):
    ...
    valid_dataloader = DataLoader(valid_data, sampler=valid_sampler, batch_size=NER_DistilBERT_BiLSTM.bs)
    ...
    pred_tags = [NER_DistilBERT_BiLSTM.tag_values[p_i] for p, l in zip(predictions, true_labels)
                                for p_i, l_i in zip(p, l) if NER_DistilBERT_BiLSTM.tag_values[l_i] != "PAD"]
    valid_tags = [NER_DistilBERT_BiLSTM.tag_values[l_i] for l in true_labels
                                for l_i in l if NER_DistilBERT_BiLSTM.tag_values[l_i] != "PAD"]
